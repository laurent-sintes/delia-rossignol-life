...
2. Exécuter `python scripts/delia_life.py offer-scan <action>`, où `<action>` vaut `clean-cache`, `full`, `delta` ou `send`.
3. Pour `clean-cache`, vérifier `cache_cleaned: true`, présenter les racines préservées et s’arrêter.
4. Pour les autres actions, conserver le manifeste `.runtime/offer-search/current.json` et utiliser strictement :
   ...
   - `revalidation_queue` comme liste prioritaire des annonces historiques à rouvrir.
...
- En mode `full`, consulter toutes les sources `core` et `complementary`, toutes les familles de requêtes et tous les secteurs prioritaires. Ne réutiliser l’historique que pour retrouver un identifiant canonique ou une provenance ; recopier dans la session uniquement les annonces effectivement revérifiées.
...
- Exécuter `python scripts/delia_life.py rank-offers <rank_inputs...> --require-complete-pool --scan-manifest .runtime/offer-search/current.json --visited-source <site> --covered-query-family <famille> --covered-priority-sector <secteur> --output <report_output_path>`. Répéter chaque option de couverture pour les éléments réellement traités.
